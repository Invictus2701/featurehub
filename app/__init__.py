...
# Note TP4 (Exo 5) : meme principe pour JWTManager. C'est l'extension qui va
# gerer les tokens JWT (création, vérification de signature, durée de vie).
jwt = JWTManager()
...
def create_app():
    ...
    # Note TP4 (Exo 5) : la JWT_SECRET_KEY est utilisée pour signer les tokens.
    # Tres important : si un attaquant la connait, il peut créer des faux tokens
    # qui passeront pour valides. En production il faut absolument la mettre dans
    # une variable d'environement et ne jamais la commiter dans git.
    app.config['JWT_SECRET_KEY'] = 'dev-jwt-secret-a-changer-en-production'
    ...
    # Note TP4 (Exo 5) : on branche JWTManager. A partir de maintenant, les
    # décorateurs @jwt_required() peuvent etre utilisés sur n'importe quelle route.
    jwt.init_app(app)
    ...
    # Note TP4 (Exo 2) : on enregistre le blueprint API sous le préfixe /api/v1.
    # Toutes les routes définies dans api/routes.py seront donc accessibles
    # sous /api/v1/... C'est ce qu'on appelle le "versioning" de l'API.
