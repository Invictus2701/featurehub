...
class FeatureRequest(db.Model):
    ...
    # ---------- Sérialisation JSON ----------
    def to_dict(self):
        """Note TP4 (Exo 2) : convertit la demande en dictionnaire Python prêt
        a etre transformé en JSON pour notre API REST.

        Pourquoi cette méthode ? Parce que Flask ne sait pas convertir un objet
        SQLAlchemy en JSON tout seul. Il a besoin d'un dict (clé:valeur) ou d'une liste.
        On fait la conversion une seule fois ici, et on l'utilise partout dans l'API.

        Note TP4 : isoformat() transforme un objet datetime en string standard
        ISO 8601 (ex: "2026-05-05T14:30:00") qui est lisible par n'importe quel
        client (navigateur, application mobile, autre serveur...).
        """
        return {
            'id': self.id,
            'title': self.title,
            ...
            # On expose juste le username, pas tout l'objet User (sécurité + simplicité)
